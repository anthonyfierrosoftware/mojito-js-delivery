  ...
  var Test = (function () {
    var constructor = function (options) {
      ...
      for (i = 0, c = keys.length; i < c; i++) {
        if (!this.options.options[keys[i]])
          this.options.options[keys[i]] = defaultOptions[keys[i]];
      }
      ...
      if (this.options.id === null)
        throw new Error("An id for this test must be specified.");
      if (this.recipes && this.recipes.length < 2)
        throw new Error("You must specify at least 2 recipes for a test.");
      if (this.options.sampleRate > 1)
        throw new Error("Sample rate must be less or equal to 1");
      ...
      if (oneRecipeRateSet && oneRecipeRateNotSet)
        throw new Error(
          "Recipe-level sampleRate property must be defined for all recipes if it is being used"
        );
      if (totalSimpleRate && totalSimpleRate !== 1)
        throw new Error("The sum of all the simple rates must be equal to 1");
    ...
    constructor.prototype = {
        ...
        if (inTest === null && isLive) {
          inTest =
            this.options.sampleRate <= 0
              ? false
              : this.getRandom() <= this.options.sampleRate;
      ...
      trackRecipeFailureEvent: function (error) {
        this.options.storageAdapter.onRecipeFailure(
          this,
          error.stack
            ? error.stack.substr(0, defaultOptions.maxErrorStackLength)
            : error.message || error
        );
      },
      /**
      ...
      setCookie: function (name, value) {
        var cookieName =
          this.options.options.cookiePrefix + "_" + this.options.id;
        ...
        Cookies.set(
          ...
          {
            expires:
              this.options.options.cookieDuration ||
              defaultOptions.cookieDuration,
          },
       ...
       */
      log: function (message, msgLevel, errorObject) {
  ...
  var Utils = {
    ...
    _observeTimeout: function (listener) {
      ...
      if (
        ...
        this.isFunction(listener.options.onTimeout)
      ) {
        listener.options.onTimeout();
      }
    },
    ...
    observeSelector: function (selector, callback, options) {
      if (defaultOptions.debug) {
        if (!this.isValidSelector(selector)) {
          throw new Error(
            "observeSelector: " + selector + " is not a valid css selector."
          );
        }

        if (!this.isFunction(callback)) {
          throw new Error(
            "observeSelector: callback parameter must be a function."
          );
        }

        if (options && !this.isObject(options)) {
          throw new Error(
            "observeSelector: options parameter must be an object."
          );
        }
      }
      ...
      if (!this.domObserver) {
        ...
        this.domObserver = new MutationObserver(
          this._observeCallback.bind(this)
        );
    ...
    _waitUntilChecker: function () {
      ...
      for (i = 0, c = this.waitListeners.length; i < c; i++) {
        ...
        if (listener.condition()) {
          ...
          if (
            listener.timeout &&
            new Date().getTime() - listener.start >= listener.timeout
          ) {
            finishedListeners.push(listener);
          }
        }
      ...
      for (i = 0, c = finishedListeners.length; i < c; i++) {
        this.waitListeners.splice(
          this.waitListeners.indexOf(finishedListeners[i]),
          1
        );
      }
      ...
      if (this.waitListeners.length) {
        this.waitHandle = window.setTimeout(
          this._waitUntilChecker.bind(this),
          defaultOptions.waitInterval
        );
      } else {
    ...
    waitUntil: function (conditionFunction, callback, timeout) {
      if (defaultOptions.debug) {
        if (!this.isFunction(conditionFunction)) {
          throw new Error(
            "waitUntil: conditionFunction parameter must be a function."
          );
        }
      ...
      if (timeout != null) {
        ...
        if (isNaN(timeout) || timeout <= 0) {
          throw new Error(
            "waitUntil: timeout parameter must be an positive Integer."
          );
        }
      } else {
    ...
    watchElement: function (selectorOrElement, callback, watchOptions) {
      ...
      if (defaultOptions.debug) {
        ...
        if (!this.isFunction(callback)) {
          throw new Error(
            "watchElement: callback parameter must be a function."
          );
        }
      }
  ...
  var Cookies = {
    ...
    get: function (name) {
      var c = document.cookie,
        ret;
      var matches = c.match(
        new RegExp("(?:^|; )" + this.escapeString(name) + "=([^;]*)")
      );
